...
public class ReservationResource {
    ...
    @Path("comments") 
    @Produces("application/json")    
    public String getAllComments(@QueryParam("user_id") String user_id, 
        ...
        if(user_id != null && post_id != null){
            ...
            return cd.getCommentsByUserId(user_id);
        } else if(post_id != null){
            return cd.getCommentsByPostId(post_id);
        } else{
            return cd.getAllComments();
        }
    ...
    @Path("comments")
    @Produces("application/json")
    public String postComment(@QueryParam("user_id") String user_id, @QueryParam("post_id") String post_id,
        ...
        if(user_id != null && post_id != null && content != null){
            ...
            return cd.postComment(user_id, post_id, content);
        }
    ...
    @Path("comments")
    @Produces("application/json")   
    public String deleteComment(@QueryParam("comment_id") String comment_id) {
        CommentsData cd;
        cd = new CommentsData(this.connection);
...
//        return this.("");     
    ...
    public String getAllMessages(){
        MessagesData md;
        md = new MessagesData(this.connection);
    ...
    @Path("messages")
    @Produces("application/json")
    public String postMessage(@QueryParam("user_id") String user_id, @QueryParam("user_name") String user_name,
            @QueryParam("content") String content){
        MessagesData md;
        md = new MessagesData(this.connection);
        if(user_id != null && user_name != null && content != null){
            return md.postMessage(user_id, user_name, content);
        }
        return "404 not valid";
    }
...
//    }
    ...
    @Path("posts")
    @Produces("application/json")
    public String deletePost(@QueryParam("post_id") String post_id){
        PostsData pd;
        pd = new PostsData(this.connection);
        pd.deletePost(post_id);
        return "";
    }
