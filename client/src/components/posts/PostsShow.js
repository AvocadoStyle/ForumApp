...
const PostsShow = (props) => {
    ...
    const deleteButtonUser = (user_id, p_id)=>{
        if(userId == user_id || userId == 1){
            // comment_id = com_id;
            return <button onClick={e=>handleDelete(e, p_id)}>delete post</button>
            // {<button>eden</button>}
        }

    }

    const handleDelete = (e, p_id) =>{
        e.preventDefault();
        let url = `http://localhost:8080/ReserveSeatWebService2/webresources/reservation/posts?post_id=${p_id}`
                ...
                {posts.map((post) => {
                    return(
                        <div key={post.post_name} className="clsborder_forum" /*className="col-lg-6 border"*/>
                            ...
                            <br></br><br></br>
                            {deleteButtonUser(post.user_id, post.post_id)}
                        </div>
                    )
                    console.log(post)
                })}
